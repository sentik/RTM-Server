...
void Streamer::startManualUpdate(streamerPlayer &player, bool getData)
{
	if (getData)
	{
...
void Streamer::executeCallbacks(const std::multimap<bool, boost::tuple<int, int> > &areaCallbacks)
{
	...
	{
		if (c->first)
			StreamerCall::OnPlayerEnterDynamicArea(c->second.get<1>(),c->second.get<0>());
		else
			StreamerCall::OnPlayerLeaveDynamicArea(c->second.get<1>(),c->second.get<0>());
	}
}
...
void Streamer::executeCallbacks(const std::vector<int> &objectCallbacks)
{
	for (std::vector<int>::const_iterator c = objectCallbacks.begin(); c != objectCallbacks.end(); ++c)
		StreamerCall::OnDynamicObjectMoved(*c);
}
